...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Deleting field 'LoadingDetail.id'
        db.delete_column('ets_loadingdetail', 'id')

        # Adding field 'LoadingDetail.slug'
        db.add_column('ets_loadingdetail', 'slug', self.gf('autoslug.fields.AutoSlugField')(unique_with=(), max_length=50, primary_key=True, default=1, unique=True, populate_from=None, db_index=True), keep_default=False)

        # Deleting field 'LoadingDetailAuditLogEntry.id'
        db.delete_column('ets_loadingdetailauditlogentry', 'id')

        # Adding field 'LoadingDetailAuditLogEntry.slug'
        db.add_column('ets_loadingdetailauditlogentry', 'slug', self.gf('autoslug.fields.AutoSlugField')(default=1, unique_with=(), max_length=50, populate_from=None, db_index=True), keep_default=False)


    def backwards(self, orm):
        
        # User chose to not deal with backwards NULL issues for 'LoadingDetail.id'
        raise RuntimeError("Cannot reverse this migration. 'LoadingDetail.id' and its values cannot be restored.")

        # Deleting field 'LoadingDetail.slug'
        db.delete_column('ets_loadingdetail', 'slug')

        # User chose to not deal with backwards NULL issues for 'LoadingDetailAuditLogEntry.id'
        raise RuntimeError("Cannot reverse this migration. 'LoadingDetailAuditLogEntry.id' and its values cannot be restored.")

        # Deleting field 'LoadingDetailAuditLogEntry.slug'
        db.delete_column('ets_loadingdetailauditlogentry', 'slug')
