...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding index on 'CompasLogger', fields ['when_attempted']
        db.create_index('ets_compaslogger', ['when_attempted'])

        # Adding index on 'Warehouse', fields ['end_date']
        db.create_index('ets_warehouse', ['end_date'])

        # Adding index on 'Warehouse', fields ['start_date']
        db.create_index('ets_warehouse', ['start_date'])

        # Adding index on 'LoadingDetail', fields ['number_of_units']
        db.create_index('ets_loadingdetail', ['number_of_units'])

        # Adding index on 'Order', fields ['created']
        db.create_index('ets_order', ['created'])

        # Adding index on 'Order', fields ['dispatch_date']
        db.create_index('ets_order', ['dispatch_date'])

        # Adding index on 'Order', fields ['expiry']
        db.create_index('ets_order', ['expiry'])

        # Adding index on 'Person', fields ['receive']
        db.create_index('ets_person', ['receive'])

        # Adding index on 'Person', fields ['dispatch']
        db.create_index('ets_person', ['dispatch'])

        # Adding index on 'ImportLogger', fields ['status']
        db.create_index('ets_importlogger', ['status'])

        # Adding index on 'ImportLogger', fields ['when_attempted']
        db.create_index('ets_importlogger', ['when_attempted'])

        # Adding index on 'OrderItem', fields ['lti_id']
        db.create_index('ets_orderitem', ['lti_id'])

        # Adding index on 'OrderItem', fields ['si_code']
        db.create_index('ets_orderitem', ['si_code'])

        # Adding index on 'OrderItem', fields ['project_number']
        db.create_index('ets_orderitem', ['project_number'])

        # Adding index on 'OrderItem', fields ['number_of_units']
        db.create_index('ets_orderitem', ['number_of_units'])

        # Adding index on 'StockItem', fields ['si_code']
        db.create_index('ets_stockitem', ['si_code'])

        # Adding index on 'StockItem', fields ['project_number']
        db.create_index('ets_stockitem', ['project_number'])


    def backwards(self, orm):
        # Removing index on 'StockItem', fields ['project_number']
        db.delete_index('ets_stockitem', ['project_number'])

        # Removing index on 'StockItem', fields ['si_code']
        db.delete_index('ets_stockitem', ['si_code'])

        # Removing index on 'OrderItem', fields ['number_of_units']
        db.delete_index('ets_orderitem', ['number_of_units'])

        # Removing index on 'OrderItem', fields ['project_number']
        db.delete_index('ets_orderitem', ['project_number'])

        # Removing index on 'OrderItem', fields ['si_code']
        db.delete_index('ets_orderitem', ['si_code'])

        # Removing index on 'OrderItem', fields ['lti_id']
        db.delete_index('ets_orderitem', ['lti_id'])

        # Removing index on 'ImportLogger', fields ['when_attempted']
        db.delete_index('ets_importlogger', ['when_attempted'])

        # Removing index on 'ImportLogger', fields ['status']
        db.delete_index('ets_importlogger', ['status'])

        # Removing index on 'Person', fields ['dispatch']
        db.delete_index('ets_person', ['dispatch'])

        # Removing index on 'Person', fields ['receive']
        db.delete_index('ets_person', ['receive'])

        # Removing index on 'Order', fields ['expiry']
        db.delete_index('ets_order', ['expiry'])

        # Removing index on 'Order', fields ['dispatch_date']
        db.delete_index('ets_order', ['dispatch_date'])

        # Removing index on 'Order', fields ['created']
        db.delete_index('ets_order', ['created'])

        # Removing index on 'LoadingDetail', fields ['number_of_units']
        db.delete_index('ets_loadingdetail', ['number_of_units'])

        # Removing index on 'Warehouse', fields ['start_date']
        db.delete_index('ets_warehouse', ['start_date'])

        # Removing index on 'Warehouse', fields ['end_date']
        db.delete_index('ets_warehouse', ['end_date'])

        # Removing index on 'CompasLogger', fields ['when_attempted']
        db.delete_index('ets_compaslogger', ['when_attempted'])
    ...
    models = {
        ...
        'ets.waybill': {
            ...
            'barcode': ('sorl.thumbnail.fields.ImageField', [], {'max_length': '255', 'null': 'True', 'blank': 'True'}),
