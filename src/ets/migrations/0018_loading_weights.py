...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for item in orm.LoadingDetail.objects.all():
            item.unit_weight_net = item.stock_item.unit_weight_net
            item.unit_weight_gross = item.stock_item.unit_weight_gross
            item.total_weight_net = ( item.number_of_units * item.stock_item.unit_weight_net ) / 1000
            item.total_weight_gross = ( item.number_of_units * item.stock_item.unit_weight_gross ) / 1000
            item.save()
