...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Deleting field 'WaybillAuditLogEntry.id'
        db.delete_column('ets_waybillauditlogentry', 'id')

        # Deleting field 'Waybill.id'
        db.delete_column('ets_waybill', 'id')

        # Changing field 'Waybill.slug'
        db.alter_column('ets_waybill', 'slug', self.gf('autoslug.fields.AutoSlugField')(unique_with=(), max_length=50, primary_key=True, unique=True, populate_from=None))


    def backwards(self, orm):
        
        # User chose to not deal with backwards NULL issues for 'WaybillAuditLogEntry.id'
        raise RuntimeError("Cannot reverse this migration. 'WaybillAuditLogEntry.id' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'Waybill.id'
        raise RuntimeError("Cannot reverse this migration. 'Waybill.id' and its values cannot be restored.")

        # Changing field 'Waybill.slug'
        db.alter_column('ets_waybill', 'slug', self.gf('autoslug.fields.AutoSlugField')(max_length=50, unique_with=(), unique=True, populate_from=None))
