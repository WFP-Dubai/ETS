...
class JSONOfflineHandler(BaseHandler):
    ...
    def read(self, request, warehouse_pk, start_date=None):
        ...
        waybills_log = ets.models.Waybill.audit_log.filter(slug__in=waybills.values_list('pk'))
        loading_details_log = ets.models.LoadingDetail.audit_log.filter(waybill__in=waybills)
        if start_date:
            ...
            loading_details_log = loading_details_log.filter(action_date__gte=start_date)
        for i in chain(ets.models.Person.objects.distinct().filter(Q(compas__warehouses__pk=warehouse_pk) |
                  ...
                  waybills_log, loading_details_log
        ):
            result.append(i)
